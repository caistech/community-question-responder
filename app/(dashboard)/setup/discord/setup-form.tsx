...
export function DiscordSetupForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const r = await fetch('/api/workspaces', {
      ...
      body: JSON.stringify({ provider: 'discord', token }),
    });
    setBusy(false);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label className="mb-1 block text-sm text-gray-300">Discord bot token</label>
        <input
          type="password"
          value={token}
        ...
        <div className="mt-1 text-xs text-gray-500">
          Bot tokens have no prefix. Don't paste a webhook URL — that's a
          different thing.
        </div>
      </div>
